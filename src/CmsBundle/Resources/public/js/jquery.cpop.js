...
(function ( $ ) {
	...
	var settings = {};
	var main_ct = null, cpop_el = null, form_el = null, cpop_hdr = null, cpop_ct = null, cpop_ft = null, cpop_bg = null, need_fixed = false, large_popup = false;
	...
	$.fn.cpop = function( options ){
		...
		this.colorPicker = function( targetEl, markupEl, markupMethod ){
			...
			cpop_ft.append( $( '<button onclick="cpop.clearColor()" type="button" class="btn-flat" id="cpop-btn-empty">' + (typeof cpop_message_empty != 'undefined' ? cpop_message_empty : 'Leeg') + '</button>' ) );
		...
		this.analyzeContent = function(){
			...
			// if( cpop_ct.find( 'form' ).length || form_el != null ){
				...
				if( cpop_ct.find( '.popup-title' ).length > 0 ){
					var pdheader = '<div class="modal-title">\
										<h3>' + cpop_ct.find( '.popup-title' ).html() + '</h3>\
									</div>\
									<div class="modal-buttons">\
										<a href="#!" onclick="cpop.close();" class="waves-effect waves-dark btn-close"><i class="material-icons">close</i></a>\
									</div>';
					cpop_hdr.html(pdheader).show();
		...
		this.setFooter = function(){
			if( handleFooter ){
				$.each( settings.buttonLabel, function( key, value ){
					if( value != '' ){
						cpop_ft.append( $( '<div class="left"><button type="button" class="btn" id="cpop-btn-' + key + '">' + value + '</button></div>' ) );
		...
		this.clear = function(){
			$('#cpop-modal .modal-content,#cpop-modal .modal-footer').html('');
		}
		...
		this.init = function(){
			...
			if( cpop_el == null ){
				cpop_el = $( '<div id="cpop-modal" class="' + (need_fixed ? 'modal-fixed-footer' : '') + ' ' + (large_popup ? 'cpop-large' : '') + ' modal' + ( settings.popupClass != '' ? ' ' + settings.popupClass : '' ) + '"></div>' );
				form_el = $( '<form id="popup-form-el" enctype="multipart/form-data" method="post"></form>' );
				cpop_ct = $( '<div class="modal-content"></div>' );
				cpop_ft = $( '<div class="modal-footer"></div>' );
				cpop_hdr = $( '<div class="modal-header"></div>' );

				main_ct.append( cpop_el );
				cpop_el.append( form_el );
				form_el.append( cpop_hdr );
				form_el.append( cpop_ct );
				form_el.append( cpop_ft );
