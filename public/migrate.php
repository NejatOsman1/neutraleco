...
$trinityConfig = $configDir . 'packages/trinity.yaml';

function parseYaml($ymlFile){
	$yaml  = new Yaml();
	return $yaml->load($ymlFile);
}

$config = parseYaml($trinityConfig);

$host = $config['parameters']['database_host'];
// if(!empty($config['parameters']['database_socket'])){ $host .= ':' . $config['parameters']['database_socket']; }
// die( "<pre>" . print_r( $host, 1 ) . "</pre>" );
$mysqli = new mysqli($host, $config['parameters']['database_user'], $config['parameters']['database_password'], $config['parameters']['database_name']);
if ($mysqli->connect_errno) { die( "<pre>" . print_r( "Failed to connect to MySQL: (" . $mysqli->connect_errno . ") " . $mysqli->connect_error, 1 ) . "</pre>" ); }

function page_bundle_reference($mysqli){
	$mutations = 0;
	echo '<p><strong>Actie: </strong>Bundle migratie in blokken en pagina\'s.</p>';

	$sql = "SELECT * FROM `page_block` WHERE `content` LIKE '%qinox%' OR `config` LIKE '%qinox%' OR `bundle_data` LIKE '%qinox%' OR `bundle` LIKE '%qinox%'";
	if($result = $mysqli->query($sql)){
		while($res = $result->fetch_assoc()){ 
            // Bundle block
            if(substr($res['bundle'], 0, 5) === 'Qinox'){
            	$res['bundle'] = preg_replace('/Qinox/', 'Trinity', $res['bundle']);
            	$res['bundle_data'] = preg_replace('/Qinox(.*?)Bundle/', 'Trinity$1Bundle', $res['bundle_data']);

            	$sql = "UPDATE `page_block` SET `bundle` = '" . $res['bundle'] . "', `bundle_data` = '" . $res['bundle_data'] . "' WHERE `id` = " . (int)$res['id'];
            	$mysqli->query($sql);
            }else{
	...
	echo '<p>Voltooid: <strong>' . $mutations . '</strong> wijzigingen.</p>';
}

if(!empty($_GET['action'])){
	switch ($_GET['action']) {
		case 'page_bundle_reference': page_bundle_reference($mysqli); break;
	}

	?>
	<p><a href="migrate.php">< Terug</a></p>
	<?php
}else{
	?>
	<h1>Trinity migratie assistent</h1>

	<p>Selecteer één van de volgende checks:</p>
	<?php
	$checks = [
		'page_bundle_reference' => 'Bundle migratie in blokken en pagina\'s.',
	];
	?>

	<select name="action" id="action" onchange="if(this.value != ''){if(confirm('Doorgaan?')){ window.location = 'migrate.php?action=' + this.value; }else{document.getElementById('action').value = '';}}">
		<option value="">- Maak een keuze -</option>
		<?php foreach($checks as $k => $v){ ?><option value="<?php echo $k; ?>"><?php echo $v; ?></option><?php } ?>
	</select>
	<?php
}
