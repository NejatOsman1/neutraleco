...
require_once __DIR__.\DIRECTORY_SEPARATOR.'FirewallConfig'.\DIRECTORY_SEPARATOR.'FosOauthConfig.php';
...
class FirewallConfig 
{
    ...
    public function fosOauth(array $value = []): \Symfony\Config\Security\FirewallConfig\FosOauthConfig
    {
        if (null === $this->fosOauth) {
            $this->_usedProperties['fosOauth'] = true;
            $this->fosOauth = new \Symfony\Config\Security\FirewallConfig\FosOauthConfig($value);
        } elseif ([] !== $value) {
            throw new InvalidConfigurationException('The node created by "fosOauth()" has already been initialized. You cannot pass values the second time you call fosOauth().');
        }
    
        return $this->fosOauth;
    }
    ...
    public function __construct(array $value = [])
    {
        ...
        if (array_key_exists('fos_oauth', $value)) {
            $this->_usedProperties['fosOauth'] = true;
            $this->fosOauth = new \Symfony\Config\Security\FirewallConfig\FosOauthConfig($value['fos_oauth']);
            unset($value['fos_oauth']);
        }
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['fosOauth'])) {
            $output['fos_oauth'] = $this->fosOauth->toArray();
        }
